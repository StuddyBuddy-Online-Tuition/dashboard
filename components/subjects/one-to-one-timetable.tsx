...
export default function OneToOneTimetable() {
  ...
  const STORAGE_KEY = "oneToOneTimetable:selectedStandards"
  ...
  type GridEntry = { subject: Subject; teacherName: string; studentName: string }
  ...
  const filteredOneToOneSlots = useMemo(() => {
    ...
    const eligibleCodes = new Set(
      selectedStandards
        .flatMap((std) => subjectsByStandard[std] ?? [])
        .filter((s) => {
  ...
  const windowsByDay = useMemo(() => {
    const result: Record<Day, TimeWindow[]> = {
      Monday: [],
      Tuesday: [],
      Wednesday: [],
      Thursday: [],
    ...
    const dayToSet: Record<Day, Set<string>> = {
      Monday: new Set(),
      Tuesday: new Set(),
      Wednesday: new Set(),
      Thursday: new Set(),
    ...
    for (const slot of filteredOneToOneSlots) {
      const label = `${slot.startTime}–${slot.endTime}`
      const day = slot.day as Day
      if (!dayToSet[day].has(label)) {
        dayToSet[day].add(label)
      }
    }
    for (const day of DAYS) {
      const labels = Array.from(dayToSet[day])
      labels.sort((a, b) => a.localeCompare(b))
      result[day] = labels.map((label) => {
        const [start, end] = label.split("–")
        return { start, end, label }
      })
    }
    return result
  }, [filteredOneToOneSlots])

  // Build grid data per standard/day/window
  const gridData = useMemo(() => {
    const result: Record<string, Record<Day, Record<string, GridEntry[]>>> = {}
    for (const standard of selectedStandards) {
      ...
      const slotsForStandard = filteredOneToOneSlots.filter((t) => subjectCodes.has(t.subjectCode))
      for (const slot of slotsForStandard) {
        const subject = subjectByCode.get(slot.subjectCode)
        if (!subject) continue
        const day = slot.day as Day
        const label = `${slot.startTime}–${slot.endTime}`
        if (!result[standard][day][label]) result[standard][day][label] = []
        result[standard][day][label].push({ subject, teacherName: slot.teacherName, studentName: slot.studentName as string })
      }
    }
    return result
  }, [selectedStandards, subjectsByStandard, subjectByCode, subjectType, filteredOneToOneSlots])
        ...
        for (const label in windows) {
          ...
          for (const entry of entries) {
            ...
            if (!map.has(baseAbbrev)) {
              const itemLabel = getBaseLabelFromSubjectField(entry.subject.subject)
              map.set(baseAbbrev, { abbrev: baseAbbrev, colorClass: getSubjectColor(baseAbbrev), name: itemLabel })
            }
          }
        }
      }
  ...
  const hasAnySelection = selectedStandards.length > 0
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-navy">1-to-1 Timetable</h2>
            <p className="text-sm text-muted-foreground">Select up to 5 standards/forms to display. Dynamic time windows are shown.</p>
          </div>
        </div>
            ...
            <table className="w-full min-w-[1000px] text-sm">
              <thead>
                <tr>
                  ...
                  {DAYS.map((day) => (
                    <th key={day} className="border px-2 py-2 text-center" colSpan={(windowsByDay[day]?.length || 0) || 1}>
                      {day}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {selectedStandards.map((standard) => (
                  <tr key={standard} className="align-top">
                    ...
                    {DAYS.flatMap((day) => {
                      const windows = windowsByDay[day]
                      const safeWindows = windows && windows.length > 0 ? windows : [{ start: "", end: "", label: "" }]
                      return safeWindows.map((tw) => {
                        const label = tw.label
                        const entries = (gridData[standard]?.[day]?.[label] ?? []) as GridEntry[]
                        return (
                          <td key={`${standard}-${day}-${label || "empty"}`} className="border px-2 py-2">
                                ...
                                entries.map((entry, idx) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <div className="opacity-80 text-[10px]">{entry.studentName} — {entry.teacherName}</div>
                                    </Link>
                                  )
                                })
                              )}
